...
# 分析爆破和目录遍历,如果请求的是同一个文件记为爆破，请求次数设为30
    ...
    for i in range(0,length):
        ...
        if int(count)>int(config.burst_num):  #记为爆破
            ...
            apache.time = times[0]
            files = file_result.read().split()[-30:]
            sqlrule = config.sqlrule # sql注入判断
            xssrule= config.xssrule #xss
            backrule=config.backrule#back
            for i in files:
                for j in sqlrule:
                    if j in i:
                        apache.attack_type = 102
            for j in xssrule:
                if j in i:
                    apache.attack_type = 101
            for j in backrule:
                if j in i:
                    apache.attack_type = 106
            apache.insert()  # 存入数据库
